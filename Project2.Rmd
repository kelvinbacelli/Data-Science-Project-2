...
We can see that of the predictor variables, age, bmi, children, and yes to being a smoker are all highly significant, with different regions being slightly significant but not by much. With all of our predictors, the adjusted R squared value came to 0.7505, meaning that just over 75% of the variation in the data can be explained by the predictors included in our model. 
...
par(mfrow=c(1,2))
hist(fit1$residuals, freq = FALSE, main = "", xlab = "Residuals") 
lines(density(fit1$residuals))

qqnorm(y=fit1$residuals, main = "")
qqline(y=fit1$residuals, datax = FALSE)
```
